...
public class OrdemDeServicoService : IOrdemDeServicoService
{
    ...
    public async Task<IEnumerable<OrdemDeServicoViewModel>> GetAllOrdens(OrdemDeServicoFilter? filter = null)
    {
		...
		if (filter != null)
		{
            ...
            if(filter.Ano != null && filter.Mes != null)
				query = query.Where(ordem => ordem.DataAbertura.Year == filter.Ano && ordem.DataAbertura.Month == filter.Mes);
		}
